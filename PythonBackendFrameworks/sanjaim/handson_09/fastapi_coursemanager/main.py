...
async def register_user(user_data: UserRegister, db: AsyncSession = Depends(get_db)):
    ...
    if not EMAIL_REGEX.fullmatch(email):
        raise_api_error(status.HTTP_422_UNPROCESSABLE_ENTITY, 'VALIDATION_ERROR', 'Invalid email format', 'email')

    existing_user_result = await db.execute(select(User).where(User.email == email))
    if existing_user_result.scalar_one_or_none():
        raise_api_error(status.HTTP_409_CONFLICT, 'CONFLICT', 'Email is already registered', 'email')
